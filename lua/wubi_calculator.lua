...
-- 说明: https://github.com/gaboolic/rime-shuangpin-fuzhuma/blob/main/md/calc.md
-- 补充说明 https://github.com/gaboolic/rime-shuangpin-fuzhuma/pull/41
...
function M.init(env)
  ...
  -- M.prefix = config:get_string(env.name_space .. '/trigger') or 'zv'
  M.prefix = "zv"
end
...
function M.func(input, seg, env)
  ...
  -- 提取算式
  if #input == 2 then
    yield(Candidate(input, seg.start, seg._end, "zv", "计算器"))
    return
  end
  local express = truncateFromStart(input, M.prefix)
  -- 算式长度 < 2 直接终止(没有计算意义)
  if (string.len(express) < 2) then
    yield(Candidate(input, seg.start, seg._end, express, "输入中"))
    return 
  end
  ...
  if success then
    yield(Candidate(input, seg.start, seg._end, result, "结果"))
    yield(Candidate(input, seg.start, seg._end, express .. "=" .. result, "等式"))
  else
    yield(Candidate(input, seg.start, seg._end, express, "解析失败"))
    yield(Candidate(input, seg.start, seg._end, code, "输入参数"))
    -- TODO: 错误信息记录到日志中
