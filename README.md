...
如果你按照前面操作，只需打开同目录中的 `ahkcmd.ahk`  即可，只支持 `ahk v2` 版本，示例如下图所示，Windows11 支持亚克力效果，图标用的是 QQ五笔 的图标。
...
- `Ctrl` + `M` 快速输入 eemv
- `Ctrl` + `H` 快速查看帮助
- `Ctrl` + `Enter` 运行命令，Esc 关闭窗口，或者切后台自动关闭窗口

![ahk界面](./sample/ahk_1.png)

### 更改配置

上面所有配置好了后，还不能直接修改词库。

- 修改同目录下的 `ini` 配置文件，打开后，改为 RIME 相应的路径
- 如果不是用的 86 版本五笔，pkl 文件也要修改，手动修改，有相应的脚本
- 定期运行 `z_the_main_dict_reordering.py` 以更新词序文件，词序文件主要是加快改词速度。
